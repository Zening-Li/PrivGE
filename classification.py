...
from models.nodepred import NodeClassifier
from layers.propagation import calc_pagerank_iters
import precompute.push as push
from datasets import Graph, load_dataset, get_labels_split_idx
...
class Instructor:
    ...
    def _load_data_init_model(self):
        # load dataset
        self.dataset = load_dataset(self.opt, 'node')
        self.labels, self.split_idx = get_labels_split_idx(self.opt['dataset_str'])
        ...
        if self.opt['propagation'] == 'power':
            ...
            # self.dataset.adj += sp.eye(num_nodes)
            ...
            self.features = torch.FloatTensor(push.calc_pagerank_push(
                ...
                num_nodes, self.opt['alpha'], self.opt['rmax'], self.opt['rrz']))
            edges = None

        self.loss_func = nn.CrossEntropyLoss()
        # multi class
        num_class = self.labels.max() + 1
        ...
        self.train_data_loader = DataLoader(
            dataset=TensorDataset(
                self.features[self.split_idx['train']],
                self.labels[self.split_idx['train']]),
            batch_size=self.opt['batch_size'], shuffle=True)
        self.model = NodeClassifier(**model_args).to(self.opt['device'])
        ...
        for epoch in range(self.opt['epochs']):
            ...
            if val_eval > max_val_eval:
                ...
                if self.opt['save'] and val_eval > self.global_eval:
                    model_path = './state_dict/' + self.opt['dataset_str'] + '_node_' + \
    ...
    def _evaluate(self, mode) -> float:
        ...
        self.model.eval()
        outputs = self.model(self.features[self.split_idx[mode]])
        y_pred = outputs.argmax(dim=-1, keepdim=True).to('cpu')
        y_true = self.labels[self.split_idx[mode]].view(-1, 1).to('cpu')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_channels', type=int, default=128, help='hidden dimensions')
    ...
    parser.add_argument('--rmax', type=float, default=5e-5, help='push threshold')
    ...
    parser.add_argument('--reg_lambda', type=float, default=5e-4, help='regularization lambda')
